...
class SongRow(ctk.CTkFrame):
    """Song Row with Album Art Square"""
...
class SettingsView(ctk.CTkFrame):
    ...
    def change_theme(self, choice):
        # 'choice' automatically grabs whatever the user clicked in the dropdown
        ctk.set_appearance_mode(choice)

class PIPPlayer(ctk.CTkFrame):
    """The 'Concept Style' PIP Player (Album Art with Overlay Controls)"""
    def __init__(self, master):
...
class App(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkFrame(self.sidebar, height=30, fg_color="transparent").pack() # Spacer
        ...
        ctk.CTkFrame(self.sidebar, height=20, fg_color="transparent").pack() # Bottom margin
    ...
    def show_frame(self, name):
        ...
        # 3. Create/Show new frame
        # (We recreate it every time to simulate a 'refresh', or you could cache them)
        frame_class = self.nav_buttons[name]["class"]
        self.current_frame = frame_class(self.main_area)
        self.current_frame.pack(fill="both", expand=True)

    def build_ui(self):
        """This is where ALL your UI code lives. No UI in __init__."""
        # -- Sidebar --
...
#center on disp only for 1080p
#only dark colours, only change text colour for now